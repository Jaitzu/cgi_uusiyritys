...
function App() {
        ...
        <MapContainer center={position} zoom={6}>
            ...
            {observationLocations.map(loc  => <Marker position={[loc.position.lon,loc.position.lat]}
                ...
                <Popup>
                    ...
                    <h1  onClick={() => setSelectedLocation(loc.info.id)}
                    >{loc && JSON.stringify(loc.info.region)}</h1>
    ...
    return (
        <div className="App">
            <pre>Double click the name of the city <br/>
                inside the marker popup</pre>
            <div selectedLocationId={selectedLocation} observationLocations={observationLocations}/>
            {map}
        </div>
    );
